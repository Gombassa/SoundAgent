...
const STEPS = [
  ...
  },
  {
    ...
    tasks: [
      {
        label: "Create the SoundAgent folder structure manually for now",
        note: "This is where the agent will deliver enriched files once built",
        code: [
          "mkdir D:\\SoundAgent",
          "mkdir D:\\SoundAgent\\_inbox",
          "mkdir D:\\SoundAgent\\_staging",
          "mkdir D:\\SoundAgent\\_errors",
          "mkdir D:\\SoundAgent\\field",
          "mkdir D:\\SoundAgent\\sfx",
          "mkdir D:\\SoundAgent\\music",
          "mkdir D:\\SoundAgent\\broadcast",
          "mkdir D:\\SoundAgent\\unclassified",
        ],
      },
      { label: "Add D:\\SoundAgent as an Import Folder in Basehead", note: "NodeTree → Imports → Add Import Folder → D:\\SoundAgent" },
      { label: "This is the Basehead delivery target — agent drops enriched files here, Basehead sees them on rescan", note: null },
      { label: "Test it: manually copy a WAV file into D:\\SoundAgent\\field", note: null },
      { label: "Right-click the SoundAgent import in NodeTree → Scan for New Files", note: "File should appear in results — confirms the delivery pipeline works" },
    ],
  },
  {
    id: 6,
    ...
    tasks: [
      { label: "Right-click the Results List column header row → select columns to display", note: "Recommended: Filename, Category, Description, Duration, Sample Rate, Bit Depth, Channels, BPM, Tags" },
  ...
  },
  {
    ...
    tasks: [
      ...
      { label: "Manually write some test metadata to it using bwfmetaedit or Basehead's own editor", note: "Category: field / Nature, Description: Test delivery from SoundAgent pipeline" },
      { label: "Copy the file into D:\\SoundAgent\\field", note: "Simulating what the agent's auto-organiser will do" },
      { label: "In Basehead: right-click SoundAgent import → Scan for New Files", note: null },
      { label: "Confirm the file appears with your test metadata visible in the columns", note: "If metadata shows correctly, the iXML embedding strategy is confirmed working" },
      { label: "Spot the file into Pro Tools via Basehead Connect", note: "Full loop confirmed: agent delivery → Basehead → DAW" },
    ],
  },
];
